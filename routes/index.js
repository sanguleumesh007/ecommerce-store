...
router.get("/", async function (req, res, next) {
  ...
  Product.find(function (err, docs) {
    console.log(docs);
    //database comes with the name docs
...
router.post("/", async function (req, res, next) {
  console.log("sdlkfjslk");
  // let insert = await Product.insertOne(lodu);
  try {
    console.log(res.body);
    const newProduct = new Product(req.body);
    const ans = await newProduct.save();
    console.log("result is : saved", ans);
    res.send("saved");
  } catch (e) {
...
router.get("/add-to-cart/:id", function (req, res) {
  //this id is that id which every product gets when it  get stored in database
...
router.post("/checkout", isLoggedIn, function (req, res, next) {
  ...
  var stripe = require("stripe")("sk_test_pVJhFSD0tie3QmfWqzusM6ib");
